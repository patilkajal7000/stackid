...
import { getDatasetIdentities } from 'core/services/DataEndpointsAPIService';
...
import DatasetIdentitiesTable from './components/DatasetIdentitiesTable';
...
const DatasetIdentities = (props: DatasetIdentitiesProps) => {
    ...
    useEffect(() => {
        dispatch(setTabsAction(SCREEN_NAME.SINGLE_DATA_ELEMENT_DATASET, NAV_TABS_VALUE.IDENTITIES, ''));
        const tabs = [
            {
    ...
    useEffect(() => {
        ...
        setIdentities([]);
        setFilteredData([]);
        getDatasetIdentities(props.cloudAccountId, props.datasetName, selectedSubTab)
            .then((response: any) => {
                setIsLoading(false);
                if (response) {
...
export default DatasetIdentities;
