...
const DataAssetActivityLog = () => {
    ...
    const [totalLogs, setTotalLogs] = useState<any>(0);
        ...
        if (dataBody) {
            ...
            dispatch(
                setTabsAction(
                    ...
                    NAV_TABS_VALUE.ACTIVITY_LOG,
                    NAV_TABS_VALUE.DATA_ENDPOINTS,
                ),
            );
            // to get the logs data
            getIdentityAWSActivityLogs(orgId, 'bucket_activity_logs', dataBody)
                .then((response: any) => {
                    ...
                    if (response) {
                        ...
                        getIdentityAWSActivityLogs(orgId, 'bucket_activity_logs_count', countBody)
                            .then((response: any) => {
                                if (response[0]?.count) {
    ...
    const onSearchIdentites = useCallback(
            ...
            if (searchString.length >= MIN_SEARCH_LENGTH) {
                const selectedItems = identityActivityLogs?.filter((data: IdentityActivityLog) => {
                    switch (selectedFilerValue) {
                        ...
                        case 5:
                            return data.IsConfigChangeEvent.toString()
                                .toLowerCase()
                ...
                if (selectedItems && selectedItems.length > 0) {
                    setTotalSearchedLogs(selectedItems?.length);
        ...
        [selectedFilerValue, identityActivityLogs],
    );
    ...
    return (
        <div className="my-4">
            <div className="container d-flex p-0">
                ...
                <SearchInput customClass="float-start w-50" onSearch={onSearchIdentites} placeholder="Search" />
                <DateRangePicker
            ...
            <ActivityLogTableComponent
                identityActivityLogs={identityActivityLogs}
...
export default React.memo(DataAssetActivityLog);
